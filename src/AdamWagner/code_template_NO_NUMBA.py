...
DECISIONS = 12  # The length of the word we are generating. Here we are generating a graph, so we create a 0-1 word of length (N choose 2)
GENERATIONS = 10000  # 1000000 generations should be plenty
...
model.compile(loss="binary_crossentropy",
              optimizer=SGD(learning_rate=LEARNING_RATE))  # Adam optimizer also works well, with lower learning rate
...
def calcScore(state):
    ...
    values_vector = [1, 2, 3, 4, 3, 2, 1, 2, 3, 4, 3, 2]
    reward = 0
    reward += (state[0] * values_vector[0] + 1) / (state[2] * values_vector[2] + 1) + (
            state[1] * values_vector[1] + 1) / (state[3] * values_vector[3] + 1)
    reward += (state[4] * values_vector[4] + 1) * (state[6] * values_vector[6] + 1) - (
            state[5] * values_vector[5] + 1) / (state[7] * values_vector[7] + 1)
    reward += (state[8] * values_vector[8] + 1) / (state[10] * values_vector[10] + 1) * (
            state[9] * values_vector[9] + 1) * (state[11] * values_vector[11] + 1)
...
for i in range(GENERATIONS):
    ...
    tic = time.time()
    sessions = generate_session(model, n_sessions,
    ...
    elite_states, elite_actions = select_elites(states_batch, actions_batch, rewards_batch,
                                                percentile=percentile)  # pick the sessions to learn from
    ...
    if (i % 20 == 1):  # Write all important info to files every 20 iterations
        with open('output/best_species_pickle_' + str(myRand) + '.txt', 'wb') as fp:
            pickle.dump(super_actions, fp)
        with open('output/best_species_txt_' + str(myRand) + '.txt', 'w') as f:
            for item in super_actions:
                f.write(str(item))
                f.write("\n")
        with open('output/best_species_rewards_' + str(myRand) + '.txt', 'w') as f:
            for item in super_rewards:
                f.write(str(item))
                f.write("\n")
        with open('output/best_100_rewards_' + str(myRand) + '.txt', 'a') as f:
            f.write(str(mean_all_reward) + "\n")
        with open('output/best_elite_rewards_' + str(myRand) + '.txt', 'a') as f:
            f.write(str(mean_best_reward) + "\n")
    if (i % 200 == 2):  # To create a timeline, like in Figure 3
        with open('output/best_species_timeline_txt_' + str(myRand) + '.txt', 'a') as f:
