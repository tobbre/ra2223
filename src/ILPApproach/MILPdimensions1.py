...
for dim in range(15, 30):
	...
	while target_lp_sol >= math.ceil((dimension + 5) / 3):
		...
		with gp.Model("MainMIP") as m:
			...
			def callback(model, where):
				if where == GRB.Callback.MIPSOL:
					...
					def cp_noSmallestOrLargest(x_, n__, s_, target_lp_sol, remove_small, remove_large):
					...
					def use_cp_noSmallestOrLargest(remove_small, remove_large):
						# This is only valid for MIRUP, not IRUP.
						# This is only valid for the 3-partition instance.
						# This is only valid if n is fixed in MainMIP, since lazy constraint does not take into account possibility of a change of n, it only takes into account forbidding patterns.
						counter = 0
						...
						status, x_used = cp_noSmallestOrLargest(x_, n__, s_, target_lp_sol, remove_small, remove_large)
						if status == 2:
							sum = 0
							counter_distinct_used_patterns = 0
							for pat in patterns:
							...
							model.cbLazy(sum <= counter_distinct_used_patterns - 1)
							# Above constraint ensures that a pattern is forbidden.
							model.update()
							return True
					...
					def isCuttingPlaneNotYetFound(B):
						if use_cp_noSmallestOrLargest(remove_small=3, remove_large=6):
							return False
						if use_cp_2nLargest_as_cuttingPlane():
							return False
						if use_cp_nMinusBtriples_as_cuttingPlane(B):
							return False
						return True
